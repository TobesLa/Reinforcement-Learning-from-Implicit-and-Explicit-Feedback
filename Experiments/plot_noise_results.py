...
# This code plots the results from the exp_noise experiment
...
noise_levels = np.arange(0, 1.2, 0.2)
num_runs = 3
...
for chosen_env in range(len(env_names)):
    ...
    for feedback_var in feedback_variants:
        file_path = os.path.join(results_directory, f"{env_name}_{feedback_var}_agents_noise_result.pkl")
...
cliff_true_rewards.get(feedback_var)[0][5]
print(cliff_true_rewards.get('IBF')[0][5])
plot_title = 'sample_IBF_cliff_noise.pdf'
#plot one sample
...
plt.xlabel('Episode')
plt.ylabel(f'Total Reward with noise of {noise_levels[5]}')
plt.legend()
plt.savefig(os.path.join(plots_directory, plot_title))
plt.show()

plot_title = 'sample_IBF_froze_noise.pdf'
#plot one sample
...
for feedback_var in feedback_variants:
    for i in range(num_runs):
        for d in range(len(noise_levels)):
...
plot_title = 'during_training_rewards_noise_cliff.pdf'
name = 'Cliffwalking'
...
for i, (ax, feedback_var) in enumerate(zip(axs, feedback_variants)):
    ...
    ax.plot(noise_levels, avg_values, label=feedback_variants[i], color=colors[i])
    ax.fill_between(noise_levels, avg_values - std_dev_values, avg_values + std_dev_values, alpha=0.25, color=colors[i])
    ax.set_xlabel('Noise')
...
plot_title = 'during_training_rewards_noise_froze.pdf'
name = 'Frozenlake'
plot_type = 'during Training'
...
#trained agent cliff
plot_title = 'trained_agent_rewards_noise_cliff.pdf'
name = 'Cliffwalking'
...
#trained agent froze
plot_title = 'trained_agent_rewards_noise_froze.pdf'
name = 'Frozenlake'
plot_type = 'on trained agent'
